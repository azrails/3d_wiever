...
float *figure_center(figure *fig){
    ...
    while(head) {
        for (int i = 0; i < head->count_point; i++) {
            ...
            if (head->array_coords[i * 3 + 1] > max_y) max_y = head->array_coords[i * 3 + 1];
            if (head->array_coords[i * 3 + 1] < min_y) min_y = head->array_coords[i * 3 + 1];
            if (head->array_coords[i * 3 + 2] > max_z) max_z = head->array_coords[i * 3 + 2];
            if (head->array_coords[i * 3 + 2] < min_z) min_z = head->array_coords[i * 3 + 2];
        }
        head = head->next;
    }
...
void rotation_figure(figure *fig, float x, float y, float z) {
    ...
    cos_x = cos(x); sin_x = sin(x);
    cos_y = cos(y); sin_y = sin(y);
    cos_z = cos(z); sin_z = sin(z);
    figure *head = fig;
    while (head){
