    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(this->ui->vertex_circle, SIGNAL(clicked()), this, SLOT(type_vertex()));
    connect(this->ui->vertex_square, SIGNAL(clicked()), this, SLOT(type_vertex()));
...
void MainWindow::on_fileDialogButton_clicked()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, tr("Chose a file"), "/home", tr("Model file (*.obj)"));
    ...
    ui->vLabelCount->setText(QString::number(this->glwidget->conf->vertex_count)); // кол-во вершин
    ui->eLabelCount->setText(QString::number(this->glwidget->conf->edge_count)); // кол-во рёбер
...
void MainWindow::on_Color_B_V_E_activated(int index) {
    if (index == 0) {
        ui->slider_red->setValue(static_cast<int>(glwidget->conf->color_fon[0] * 255));
        ui->slider_green->setValue(static_cast<int>(glwidget->conf->color_fon[1] * 255));
        ui->slider_blue->setValue(static_cast<int>(glwidget->conf->color_fon[2] * 255));
        ui->slider_size_v_e->setValue(1);
    } else if (index == 1) {
        ui->slider_red->setValue(static_cast<int>(glwidget->conf->color_point[0] * 255));
        ui->slider_green->setValue(static_cast<int>(glwidget->conf->color_point[1] * 255));
        ui->slider_blue->setValue(static_cast<int>(glwidget->conf->color_point[2] * 255));
        ui->slider_size_v_e->setValue(glwidget->conf->point_width);
    } else {
        ui->slider_red->setValue(static_cast<int>(glwidget->conf->color_line[0] * 255));
        ui->slider_green->setValue(static_cast<int>(glwidget->conf->color_line[1] * 255));
        ui->slider_blue->setValue(static_cast<int>(glwidget->conf->color_line[2] * 255));
...
void MainWindow::on_rotateButton_clicked() {
    ...
    if (ok && this->glwidget->conf->fig) {
        ...
        float *center_before_rotate = figure_center(glwidget->conf->fig); //центры коор. фигуры
        transform_coordinates(glwidget->conf->fig, center_before_rotate);
        if (index == 0) rotation_figure(this->glwidget->conf->fig, angle, 0, 0);
        else if (index == 1) rotation_figure(this->glwidget->conf->fig, 0, angle, 0);
        else rotation_figure(this->glwidget->conf->fig, 0, 0, angle);
        float *center_after_rotate = figure_center(glwidget->conf->fig); // новый центр фигуры
        for (int i = 0; i < 3; i++) {
...
void MainWindow::on_translateButton_clicked()
{
    ...
    if (ok && this->glwidget->conf->fig) {
        ...
        if (index == 0) translate_figure(this->glwidget->conf->fig, value, 0, 0);
        else if (index == 1) translate_figure(this->glwidget->conf->fig, 0, value, 0);
        else translate_figure(this->glwidget->conf->fig, 0, 0, value);
...
void MainWindow::on_scaleButton_clicked()
{
    ...
    if (ok && this->glwidget->conf->fig) {
        ...
        float *center_before_scaling = figure_center(glwidget->conf->fig); //центры коор. фигуры
        transform_coordinates(glwidget->conf->fig, center_before_scaling);
        if (index == 0) scale_figure(this->glwidget->conf->fig, value, 1, 1);
        else if (index == 1) scale_figure(this->glwidget->conf->fig, 1, value, 1);
        else scale_figure(this->glwidget->conf->fig, 1, 1, value);
        float *center_after_scaling = figure_center(glwidget->conf->fig); // новый центр фигуры
        for (int i = 0; i < 3; i++) {
            center_before_scaling[i] = -center_before_scaling[i] + center_after_scaling[i];
        }
        transform_coordinates(glwidget->conf->fig, center_before_scaling);
